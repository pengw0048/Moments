...
Public Class frmPersonType
    ...
    Sub updateList()
        ...
        Dim cmd As New MySqlCommand("SELECT ID, TName, Des FROM persontype", frmMain.conn)
        Try
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If MsgBox("真的要删除吗？", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            Dim cmd As New MySqlCommand("DELETE FROM persontypify WHERE TypeID=" + Trim(idList(ListBox1.SelectedIndex).ToString), frmMain.conn)
            Try
                cmd.ExecuteScalar()
            Catch ex As Exception
                MessageBox.Show("写入错误：" + ex.Message)
            End Try
            cmd = New MySqlCommand("DELETE FROM persontype WHERE ID=" + Trim(idList(ListBox1.SelectedIndex).ToString), frmMain.conn)
            Try
                cmd.ExecuteScalar()
            Catch ex As Exception
                MessageBox.Show("写入错误：" + ex.Message)
